...
extension GeoDrawer {
  ...
  func draw(_ bounds: MapBounds, fillColor: CGColor? = nil, strokeColor: CGColor? = nil, in context: CGContext) {
    ...
    switch bounds {
    case .ellipse:
      let min = projection.translate(.init(x: -1 * projection.projectionSize.width / 2, y: projection.projectionSize.height / 2), zoomTo: zoomTo, to: size)
      let max = projection.translate(.init(x: projection.projectionSize.width / 2, y: -1 * projection.projectionSize.height / 2), zoomTo: zoomTo, to: size)
    ...
    case .bezier(let array):
      let points = array.map { projection.translate($0, zoomTo: zoomTo, to: size) }
